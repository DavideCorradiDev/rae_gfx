...
impl std::fmt::Display for EndFrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EndFrameError::NotProcessingFrame => write!(f, "No frame is being processed"),
            EndFrameError::ImageAcquisitionFailed => write!(f, "Failed to acquire image"),
...
impl std::fmt::Display for SynchronizeFrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SynchronizeFrameError::OutOfMemory(e) => write!(f, "Out of memory ({})", e),
...
impl From<hal::device::OomOrDeviceLost> for SynchronizeFrameError {
    fn from(e: hal::device::OomOrDeviceLost) -> Self {
        match e {
            hal::device::OomOrDeviceLost::OutOfMemory(e) => SynchronizeFrameError::OutOfMemory(e),
            hal::device::OomOrDeviceLost::DeviceLost(e) => SynchronizeFrameError::DeviceLost(e),
        }
    }
}
